...
- **`criticPass`** (`sections/critic.ts`) — forces `submit_corrected_section` returning `{corrected: string}`. The model has no text channel to leak essays, meta-commentary, or "The correct AA section is:" prefixes. Empty-section = pass `""`. Accepts an optional `model: "haiku" | "sonnet"` tier. The hard sections — **Záver**, **OA**, **LA** — route to **Sonnet 4.6** via `runCriticAndReconcilers` (membership check on `SONNET_CRITIC_LABELS` in `pipeline.ts`); every other section stays on Haiku. Rationale: Záver needs chained ICD inference (I21.2 lateral over I21.9 unspecified, E89.0 from "st.p. strumektómii + Euthyrox", I34.0 over I35.x for mitral regurg, D47.2 for MGUS); OA preserves dense Slovak shorthand lists (ICHS, DM 2, DLP, st.p. CHE, BPH, VAS) Haiku occasionally normalises or drops; LA preserves drug names verbatim (no "Minivi Meld" hallucinations). Cost: ~$0.09/visit extra for 3 Sonnet calls. Latency: no measurable impact — critic calls run in parallel. Eval: bumped from 75/84 → 77-78/84 (the Záver ICD wins are the measurable portion; OA/LA are insurance for production quality).

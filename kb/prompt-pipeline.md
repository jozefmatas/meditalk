...
- **`criticPass`** (`sections/critic.ts`) — forces `submit_corrected_section` returning `{corrected: string}`. The model has no text channel to leak essays, meta-commentary, or "The correct AA section is:" prefixes. Empty-section = pass `""`. Accepts an optional `model: "haiku" | "sonnet"` tier: **Záver** is routed to **Sonnet 4.6** via `runCriticAndReconcilers` because its ICD anatomy + comorbidity inference benefits from the stronger reasoning tier (picks I21.2 lateral over I21.9 unspecified, E89.0 from "st.p. strumektómii + Euthyrox", I34.0 over I35.x for mitral regurg, D47.2 for MGUS). Every other section stays on Haiku. Adds ~$0.03/visit, no measurable latency impact. Bumped eval from 75/84 → 77-78/84.

...
| `STRUCTURAL_VITAL_LABELS` + `EXAM_NARRATIVE_LABELS` (normalised label sets)               | [`web/src/lib/sections/kind-policy.ts`](../web/src/lib/sections/kind-policy.ts)        | Add locale's label spellings (e.g. German "Größe", "Gewicht"; French "Taille", "Poids")      | Add specialty-specific sections if they need the same voice-example skip treatment             |
| `CONCLUSION_LABELS` (recognised conclusion labels)                                        | [`web/src/lib/sections/kind-policy.ts`](../web/src/lib/sections/kind-policy.ts)        | Add locale's "Assessment" / "Závěr" / "Fazit" / "Conclusion" variants                        | None                                                                                           |
...
```
  ...
  Stage 2      parallel (started immediately):
               ...
               |        filterSourceForKind(source, kind)     sections/kind-policy.ts
...
| [`web/src/lib/sections/reconcilers/index.ts`](../web/src/lib/sections/reconcilers/index.ts) | Registry: `drug-normalizer`, `icd-validator`. Each reconciler has signature `(text, source, { language }) => string`. The `drug-substitution-guard` runs universally (not registered per-template) as a first-pass in `applyReconcilers()`.                                                                                                                         |
...
| [`web/src/lib/lookup/medications.ts`](../web/src/lib/lookup/medications.ts) | Medication CSV loader — `isValidMedication`, `isActiveIngredient`, `getActiveIngredient`, `correctMedicationBaseName` (drug-normalizer reconciler), `searchMedications` (admin panel). |
...
| -------------------------- | ------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `drug-substitution-guard`  | **All sections (universal first-pass)**            | Deterministic guard that catches LLM brand↔generic drug name substitutions. Compares drug name prefixes in the draft against the source (files + transcript + doctor notes). Two detection strategies: (1) same active ingredient via CSV lookup (e.g. Fokusin → Tamsulosín share ingredient "tamsulosín"), (2) name containment for combination brands (e.g. Co-Prenessa contains Prenessa). Replaces substituted names with the source's original. Runs as a universal first-pass in `applyReconcilers()` before any per-section reconcilers — not registered in `RECONCILERS` map. See [`drug-substitution-guard.ts`](../web/src/lib/sections/reconcilers/drug-substitution-guard.ts). |
...
- **Wrong dose/drug name in LA** — the `drug-normalizer` alias map in [`reconcilers/drug-normalizer.ts`](../web/src/lib/sections/reconcilers/drug-normalizer.ts) catches known Slovak shortforms (ANP, ASA, NTG). Add to `ABBREVIATION_ALIASES` when a new one surfaces. If a generic name (e.g. "Ramipril") is being expanded to a branded variant with manufacturer (e.g. "Ramipril Actavis"), the guard in `correctMedicationBaseName` (`medications.ts`) should prevent this — check the `startsWith` logic. Brand↔generic substitution (e.g. Tamsulosín → Fokusin, Prenessa → Co-Prenessa) is caught deterministically by the `drug-substitution-guard` reconciler in [`reconcilers/drug-substitution-guard.ts`](../web/src/lib/sections/reconcilers/drug-substitution-guard.ts) — it runs as a universal first-pass in `applyReconcilers()` before any per-section reconcilers. Two detection strategies: (1) same active ingredient via CSV lookup, (2) name containment for combination brands. The section-agent and critic prompts also ban substitution explicitly.

...
| File                                                                                        | Role                                                                                                                                                                                                                                                                                                                                                                |
| ------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`web/src/lib/sections/kind-policy.ts`](../web/src/lib/sections/kind-policy.ts)              | `KIND_POLICY` matrix (kind → render model, critic model, digit grounding, voice-example suppression), `CATEGORY_ROUTING` (kind → passage categories), `resolveKind`, `resolveKindFromTitle`, `filterSourceForKind`. Importable without pulling in the full orchestrator. |
| [`web/src/lib/sections/pipeline.ts`](../web/src/lib/sections/pipeline.ts)                   | Orchestrator. Walks template leaves: non-conclusion first (LLM render → critic → reconcilers), conclusion last (deterministic `formatConclusionContent`). `findConclusionSection`, `formatConclusionContent`, `runCriticAndReconcilers` exported. Re-exports `KIND_POLICY`, `CATEGORY_ROUTING`, `resolveKind`, `filterSourceForKind` from `kind-policy.ts`. |

...
- **Web path.** Uses the browser `MediaRecorder` API with native `pause()`/`resume()` to maintain a single container across pause/resume cycles — `stop()` produces one valid file containing all audio. Preferred MIME types, in order: `audio/webm;codecs=opus`, `audio/webm`, `audio/ogg;codecs=opus`, `audio/ogg`, `audio/mp4`. If none is supported (rare), the fallback is `audio/mp4` (not empty string — passing `""` as `mimeType` causes `NotSupportedError` on some browsers). At pause time, `requestData()` flushes accumulated chunks before pausing — with a feature-detection guard + 500ms timeout because `requestData()` is not reliably supported on all browsers (Safari iOS has historically been inconsistent). `resume()` also guards against calling `recorder.resume()` when the recorder isn't in `"paused"` state, which can happen when `requestData()` fails to trigger `ondataavailable` before the timeout. `getSnapshotBlob()` returns the accumulated audio without stopping the recorder.
...
- The client-side helper is [transcribeBlob](web/src/components/encounters/hooks/transcribe-blob.ts) — the primary transcription path for all recordings. Includes a **single retry** with 2s delay for transient HTTP errors (408, 429, 502, 503, 504) and `TypeError` (network failure). The upload filename is derived from the blob's actual MIME type via `blobMimeToExt()` (`audio/mp4` → `.m4a`, `audio/ogg` → `.ogg`, `audio/wav` → `.wav`, default `.webm`) — this is critical because Safari records `audio/mp4` and sending it with a `.webm` filename confuses server-side format detection (ElevenLabs).
- The server endpoint [batch-transcribe/route.ts](web/src/app/api/batch-transcribe/route.ts) also uses `.m4a` as the fallback filename (not `.webm`) since Safari is the most common source of unlabeled audio.
...
| `requestData()` not firing `ondataavailable` on Safari iOS (pause-time data loss)| Feature-detection guard + 500ms timeout ensures pause completes; `resume()` guards against non-paused state.                           |
...
| Server dying mid-generation (encounter stuck in "processing")                   | `generation_pending.startedAt` timestamp + client auto-correction resets to "started" after 5 min. Polling hook also has 300s timeout. |

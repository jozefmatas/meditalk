...
| OpenAI embeddings  | [web/src/lib/openai.ts](web/src/lib/openai.ts)                                                                                 | `embedText` / `embedTexts`. Default model: `text-embedding-3-small` (1536 dims). Legacy chunk queries pass `LEGACY_EMBEDDING_MODEL` (`text-embedding-ada-002`) to stay compatible with stored vectors.                                                                                                                        |
...
6. **Embedding search.** `transcript_chunks` is now fully deprecated for reads — the regenerate route prefers `metadata.transcript` and only falls back to chunks for pre-migration encounters. The table still exists for legacy similarity search but no new data is written to it. Legacy queries use `text-embedding-ada-002` to stay compatible with stored vectors; new code defaults to `text-embedding-3-small`. Decide whether to drop the table entirely once all legacy encounters are backfilled.

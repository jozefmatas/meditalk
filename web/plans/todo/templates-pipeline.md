...
*Smallest change with biggest impact — makes prompts per-template configurable.*
...
*Style examples let us feed reference notes into the prompt.*
...
*Move templates from static code to the database.*
...
*Frontend can fetch templates from DB, gracefully falling back to static.*
...
*See and manage templates from admin.*
...
*Build/edit template sections from admin.*
...
*Edit the system prompt per template from admin.*
...
*Upload reference documents, AI extracts text, injected into prompt.*
